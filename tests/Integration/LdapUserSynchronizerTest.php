...
namespace Rocket\Core\Tests\Integration;

use Rocket\Core\Entity\User;
use Rocket\Core\Enum\UserSource;
use Rocket\Core\Ldap\DirectoryUser;
use Rocket\Core\Ldap\LdapDirectory;
use Rocket\Core\Ldap\LdapUserSynchronizer;
...
final class LdapUserSynchronizerTest extends KernelTestCase
{
    ...
    private function synchronizer(array $entries): LdapUserSynchronizer
    {
        $directory = new class($entries) implements UserDirectoryInterface {
            ...
            public function probe(\Rocket\Core\Ldap\LdapConfig $config, int $limit = 5): array
            {
                return ['count' => \count($this->entries), 'sample' => \array_slice($this->entries, 0, $limit)];
            }

            public function ping(\Rocket\Core\Ldap\LdapConfig $config): void
            {
            }
        };
    ...
    public function testEmptyPasswordNeverReachesTheDirectory(): void
    {
        ...
        $settings = new class extends \Rocket\Core\Ldap\LdapSettings {
            public function __construct()
            {
            }

            public function get(): \Rocket\Core\Ldap\LdapConfig
            {
                return new \Rocket\Core\Ldap\LdapConfig(enabled: true, url: 'ldap://unreachable.invalid', baseDn: 'dc=ex');
            }
        };
