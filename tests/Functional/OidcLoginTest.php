...
final class OidcLoginTest extends WebTestCase
{
    ...
    public function testProvidersAreListedPublicly(): void
    {
        ...
        self::assertSame('rocket-app', $response['providers'][0]['clientId']);
    ...
    public function testAdminsConfigureProvidersWithAnEncryptedSecret(): void
    {
        ...
        $created = $this->api('POST', '/api/authentication_servers', [
            ...
            'clientId' => 'rocket-app', 'clientSecret' => 'top-secret', 'scopes' => 'openid  email profile',
        ], $auth);
    ...
    private function createServer(string $name = 'Rocket Auth', bool $enabled = true, string $adminGroup = ''): AuthenticationServer
    {
        $server = (new AuthenticationServer())
            ...
            ->setClientId('rocket-app')
            ->setEncryptedClientSecret(static::getContainer()->get(\Rocket\Core\Security\SecretBox::class)->encrypt('s3cret'))
    ...
    {
        $idToken = Jwt::sign($claims + ['iss' => self::ISSUER, 'aud' => 'rocket-app', 'iat' => time(), 'exp' => time() + 300], $key ?? $this->key, Jwt::publicJwk($this->key)['kid']);
