...
final class DemoSeedCommandTest extends WebTestCase
{
    ...
    public function testDeclaresTheSingleSignOnProviderWhenConfigured(): void
    {
        ...
        self::assertSame('demo-secret', $container->get(\Rocket\Core\Security\SecretBox::class)->decrypt($servers[0]->getEncryptedClientSecret()));
    ...
    private function command(bool $demoMode, string ...$sso): DemoSeedCommand
    {
        ...
        return new DemoSeedCommand(
            ...
            $container->get(\Rocket\Core\Repository\UserRepository::class),
            $container->get(\Rocket\Core\Repository\ApplicationRepository::class),
            $container->get(\Rocket\Core\Repository\AuthenticationServerRepository::class),
            $container->get(\Symfony\Component\PasswordHasher\Hasher\UserPasswordHasherInterface::class),
            $container->get('security.token_storage'),
            $container->get(\Rocket\Core\Security\SecretBox::class),
            [],
            $demoMode,
            self::DEMO_TOKEN,
            ...$sso,
