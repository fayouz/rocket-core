...
useHead({ title: `Palettes · ${appName}` })
...
<template>
  <UDashboardPanel id="palettes">
    ...
    <template #body>
      <div class="grid gap-6 xl:grid-cols-[340px_1fr]">
        <div class="flex flex-col gap-3">
          <p class="text-sm text-muted">
            La palette du projet habille {{ appName }}, page de connexion comprise.
            <template v-if="embed">
              Par défaut, elle habille aussi les pages embarquées dans les applications ; une application peut avoir sa propre palette (page Applications).
            </template>
          </p>
          <button
            type="button"
          ...
          >
            ...
            <span class="min-w-0 flex-1 text-sm">Couleurs par défaut de {{ appName }}</span>
        ...
        <form v-if="selectedId" class="flex min-w-0 flex-col gap-4" data-testid="palette-form" @submit.prevent="save">
          ...
          <UFormField label="Aperçu">
            <div data-palette-preview class="flex flex-col gap-3 rounded-lg border border-default p-4" data-testid="palette-preview">
              ...
              <UAlert color="primary" variant="subtle" icon="i-lucide-sparkles" title="Votre message est prêt" description="Aperçu des couleurs avant d’enregistrer." />
      ...
      <UModal :open="toDelete !== null" title="Supprimer la palette ?" :description="toDelete ? `Si « ${toDelete.name} » est la palette du projet, il reprend ses couleurs par défaut ; les applications qui l’utilisent reprennent la palette du projet.` : ''" @update:open="(value: boolean) => { if (!value) toDelete = null }">
        <template #footer>
