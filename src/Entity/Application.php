...
class Application
{
    /** Prefix of the secret tokens, one per application (rma_ for Rocket Mailer, rpa_ for Rocket Print…): rocket_core.token_prefix. */
    private static string $tokenPrefix = 'rka_';
    ...
    /** @internal set by RocketCoreBundle::boot() */
    public static function setTokenPrefix(string $prefix): void
    {
    ...
    {
        $token = self::$tokenPrefix.bin2hex(random_bytes(32));
    ...
    {
        if (!str_starts_with($token, self::$tokenPrefix) || \strlen($token) < 36) {
            throw new \InvalidArgumentException(\sprintf('An application token must start with "%s" and be at least 36 characters long.', self::$tokenPrefix));
        }
